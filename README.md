...
管理页面只供开服设备使用，当然用别的设备访问也是可以的。其他玩家打开控制台列出的局域网游戏地址即可；其他玩家也可以在下载本项目后，打开自己的`public/game.html`，选择`局域网对战`，再填写服务器IP、端口和五位登录码。若浏览器限制本地HTML访问局域网，可以改用服务器提供的`http://IP:端口/game.html`。
...
1. 选择`局域网对战`，填写IP、端口和玩家A登录码，也可以将全部内容粘贴到智能输入。
2. 根据需要进行设置。
...
1. 选择`局域网对战`，填写IP、端口和玩家B登录码，也可以将全部内容粘贴到智能输入。

(function () {
  ...
  // Small test/debug surface used by the local smoke test.
  window.__doubleFlightDebug = {
    getEngine: () => engine,
    startClassic: (colors = ['red', 'yellow']) => {
      mode = 'classic';
      playerAColors = colors.slice(0, 2);
      protectedColors.clear();
      firstPlayer = 'A';
      startGame();
      return engine;
    ...
    animateMovementEvents,
    getPalette: () => JSON.parse(JSON.stringify(palette)),
    getTiming: () => ({ ...timing }),
    showVictoryPopup,
    renderPieces,
    expandedOverlapLayout,
    getExpandedOverlapClientRegion,
    getOverlapRegions: () => new Map(overlapHitRegions),
    getPendingConfirmation: () => pendingConfirmation ? cloneData(pendingConfirmation) : null,
    getPendingSwapOrderChoice: () => pendingSwapOrderChoice ? cloneData(pendingSwapOrderChoice) : null,
    getLockedOverlapKey: () => lockedOverlapKey,
    renderConfirmationArrows,
    getUndoRecord: () => undoRecord ? { kind: undoRecord.kind, snapshot: JSON.parse(JSON.stringify(undoRecord.snapshot)) } : null,
    getAssignmentCheckpoint: () => assignmentCheckpoint ? JSON.parse(JSON.stringify(assignmentCheckpoint)) : null,
    getLanChatMessages: () => cloneData(lanChatMessages),
    getLanChatSendKeyMode: () => lanChatSendKeyMode,
    getNetworkInterval: () => lanClient ? lanClient.intervalMs : null,
    parseSmartLanInput: text => ({ ip: parseIpv4(text), port: text.match(/(?:^|\D)(\d{4})(?!\d)/)?.[1] || '', code: text.match(/(?:^|\D)(\d{5})(?!\d)/)?.[1] || '' }),
    getLanVisualVersion: () => lanVisualVersion,
    isLanAnimationActive: () => lanAnimationActive,
    launchLanSpirit,
    getColumnWidths: () => ({ left: el.leftPanel?.getBoundingClientRect().width || 0, right: el.turnInteractionPanel?.getBoundingClientRect().width || 0 }),
    getAiModels: () => window.DoubleFlightAI ? window.DoubleFlightAI.modelOptions.slice() : []
  };
})();

(function(){
  ...
  let latest=null,selectedRoomId=null,pendingImportRoomId=null,chatSendKeyMode='enter',copyTimer=null,confirmResolve=null,confirmReturnFocus=null;
  ...
  function settleConfirm(accepted){const resolve=confirmResolve;confirmResolve=null;$('adminConfirmModal').classList.add('hidden');if(resolve)resolve(Boolean(accepted));const target=confirmReturnFocus;confirmReturnFocus=null;if(target&&typeof target.focus==='function')requestAnimationFrame(()=>target.focus())}
  function askConfirm(message,acceptLabel='确认'){if(confirmResolve)settleConfirm(false);confirmReturnFocus=document.activeElement;$('adminConfirmMessage').textContent=String(message||'');$('adminConfirmAccept').textContent=String(acceptLabel||'确认');$('adminConfirmModal').classList.remove('hidden');requestAnimationFrame(()=>$('adminConfirmCancel').focus());return new Promise(resolve=>{confirmResolve=resolve})}
  ...
  async function roomAction(roomId,action){if(action==='close'&&!(await askConfirm(`确定关闭房间${roomId}？正在游戏的玩家会立即失去连接。`,'关闭房间')))return;try{if(action==='export'){location.href='/api/admin/export-game?roomId='+encodeURIComponent(roomId)+'&download='+Date.now();return}if(action==='import'){pendingImportRoomId=Number(roomId);$('importFile').value='';$('importFile').click();return}const paths={open:'/api/admin/room/open',restart:'/api/admin/room/restart',refresh:'/api/admin/room/refresh-codes',close:'/api/admin/room/close'};paint(await api(paths[action],{roomId:Number(roomId)}));selectedRoomId=Number(roomId);notice(`房间${roomId}操作完成`)}catch(error){notice('操作失败：'+error.message);await status()}}
  ...
  $('aboutButton').onclick=()=>{$('aboutModal').classList.remove('hidden');requestAnimationFrame(()=>$('closeAboutButton').focus())};$('closeAboutButton').onclick=()=>$('aboutModal').classList.add('hidden');$('aboutModal').onclick=event=>{if(event.target===$('aboutModal'))$('aboutModal').classList.add('hidden')};$('adminConfirmCancel').onclick=()=>settleConfirm(false);$('adminConfirmAccept').onclick=()=>settleConfirm(true);$('adminConfirmModal').onclick=event=>{if(event.target===$('adminConfirmModal'))settleConfirm(false)};document.addEventListener('keydown',event=>{if(event.key!=='Escape')return;if(!$('adminConfirmModal').classList.contains('hidden')){event.preventDefault();settleConfirm(false)}else if(!$('aboutModal').classList.contains('hidden')){$('aboutModal').classList.add('hidden')}});
